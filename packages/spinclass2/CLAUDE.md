...
Shell-agnostic git worktree session manager (development fork of spinclass).
Manages worktree lifecycles: creating worktrees with config inheritance,
attaching via configurable session entrypoints, rebasing/merging back to main,
and cleaning up. Aliased as `sc-dev`. Part of the bob monorepo.
...
``` sh
just build-spinclass2   # nix build .#spinclass2
just test-spinclass2    # Go tests with TAP-14 output
```

Tests run through the monorepo root's devShell and use `tap-dancer` for TAP-14
formatted output.
...
## CLI Commands
  ...
  Command                   Description
  ------------------------- -----------------------------------------------------
  `sc-dev attach [name]`    Create (if needed) and attach to a worktree session
  `sc-dev list`             List all tracked sessions from state directory
  `sc-dev merge [target]`   Merge worktree into main, remove session state
  `sc-dev clean`            Remove merged worktrees and abandoned sessions
  `sc-dev fork [branch]`    Fork current worktree (supports `--from <dir>`)
  `sc-dev pull`             Pull repos and rebase worktrees
  `sc-dev validate`         Validate sweatfile hierarchy
  `sc-dev exec-claude`      Run claude with sweatfile settings

## Nix Build

Built via `lib/packages/spinclass2.nix` using `mkGoWorkspaceModule`. Binary
installs as `spinclass2` with `sc-dev` symlink.

## Go Workspace

Module: `github.com/amarbel-llc/spinclass2`. Uses `replace` directive for local
`tap-dancer/go` dependency. Part of the monorepo `go.work` workspace.
