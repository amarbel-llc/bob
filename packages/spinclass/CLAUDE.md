...
``` sh
just build-spinclass   # nix build .#spinclass
just test-spinclass    # Go tests with TAP-14 output
```
...
Built via `lib/packages/spinclass.nix` using `mkGoWorkspaceModule`. Binary
installs as `spinclass` with `sc` symlink.

## Go Workspace

Module: `github.com/amarbel-llc/spinclass`. Uses `replace` directive for local
